/* Sub-module: left-panel/items/gif — GIPHY GIF slider + drill-in (search,
   infinite scroll). Fetch/add via the shared ItemsCore engine. */
(function () {
  'use strict';
  if (!window.cc || !cc.modules) { console.warn('[items.gif] cc skeleton missing'); return; }
  ...
  function fillSlider(slider) {
    ...
    c.giphyFetch('gifs', 'trending', '', 0, 15, function (items) {
      ...
      items.forEach(function (g) {
        ...
        cell.addEventListener('click', function () { c.addGiphy(g, 'gif'); });
  ...
  function renderDrill() {
    ...
    var search = view.querySelector('.items-gif-search');
    if (!grid) return;
  ...
  var _reg = false;
  function mount() { if (_reg) return; _reg = true; if (window.ItemsCore) ItemsCore.registerTab({ key: 'gif', label: 'GIFs', order: 3, fillSlider: fillSlider, renderDrill: renderDrill }); }

  cc.modules.register({ id: 'gif', parent: 'left-panel.items', title: 'GIFs', mount: mount, unmount: function () {} });
})();
